...
import com.codeapes.checklist.domain.ChecklistGroupType;
...
public class TestDataController {
    ...
    private static final int NUMBER_OF_EXEC_CHECKLIST_STATES = 3;
    ...
    private void createSetOfChecklists(int start, OwnerExecutor owner, ChecklistGroup group, String createdBy) {
        for (int i = start; i < (start + NUMBER_OF_CHECKLISTS_TO_GENERATE); i++) {
            ...
            createExecutableChecklist(checklist, owner, createdBy, i - start);
        }
    }

    private void createExecutableChecklist(Checklist checklist, OwnerExecutor executor, String createdBy, int i) {
        final Checklist executableChecklist = checklistService
                .createExecutableChecklist(checklist, executor, createdBy);
        executableChecklist.setStatus(determineExecutionStatus(i));
        final Timestamp currentDateTime = new Timestamp(new Date().getTime());
        executableChecklist.getExecutionInfo().setExecutionStart(currentDateTime);
        executableChecklist.getExecutionInfo().setExecutionEnd(currentDateTime);
        checklistService.updateChecklist(executableChecklist, createdBy);
    }

    private ChecklistStatus determineExecutionStatus(int i) {
